...
router.post('/create-case', authenticateToken, authorizeRoles('SUPER_ADMIN', 'SUB_INSPECTOR'), asyncHandler(async (req: any, res: any) => {
  const { id, title, crimeType, priority, location, firId } = req.body;
    ...
    data: {
      ...
      timeline: {
        create: [
          ...
          { step: 'Officer Assignment & Briefing', completed: false, details: 'Awaiting Superintendent supervisor assignment.' }
        ]
      },
      caseNotes: {
        create: [
          { note: 'Case file generated from preliminary FIR statements. Ready for Inspector assignment.', author: req.user.name }
        ]
      },
      firId
    }
  ...
  await logWorkflowAction(req, req.user.officerId, req.user.role, 'Case Created', `Case ${id} created from FIR ${firId}.`, id);
  
  await NotificationService.notifyAll(`Case Opened: New criminal case "${title}" (ID: ${id}) initialized.`, 'Info').catch(console.error);
